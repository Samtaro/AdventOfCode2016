var rooms = `fubrjhqlf-edvnhw-dftxlvlwlrq-803[wjvzd]
...
var roomsToArr = rooms.split("\n")
var realRooms = roomsToArr.map(function(room){
  //make the rooms into nice objects
  return {
    name: room.substring(0, room.lastIndexOf("-")).replace(/[-]/g, ""),
    id: room.substring(room.lastIndexOf("-")+1, room.indexOf("[")),
...
.filter(function(room){
  ...
  for(var i = 0; i < room.name.length; i++){
    if(!letterCounter[room.name.charAt(i)]){
      letterCounter[room.name.charAt(i)] = 1;
    }
    else{
      letterCounter[room.name.charAt(i)] += 1;
    }
  }
  //organize letters by occurences
  var lCounterSort = {};
  //push occurences to array
  var occurence = []
  for(var key in letterCounter){
    if(!lCounterSort[letterCounter[key]]){
      occurence.push(letterCounter[key])
      lCounterSort[letterCounter[key]] = [key];
    }
    else{
      lCounterSort[letterCounter[key]].push(key);
    }
  }
  occurence.sort(function(a,b){return b-a})
  for(var key in lCounterSort){
    lCounterSort[key].sort();
  }
  var check = ""
  for(var i = 0; i < occurence.length; i++){
    for(var j = 0; j < lCounterSort[occurence[i]].length; j++){
    if(check.length<5){
      check += lCounterSort[occurence[i]][j]
      }
    }
  }
