...
for(var i= 0; i< screen[0].length; i+=5){
var secretMessage = screen[0].slice(i,i+5).join().replace(/[,]/g, "")
for(var j=1; j< screen.length; j++){
  secretMessage += "\n" + screen[j].slice(i,i+5).join().replace(/[,]/g, "")
}
